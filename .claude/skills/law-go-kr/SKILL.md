...
- 조문 구조화는 법제처가 주는 형태에 따라 결과가 달라진다. `조문단위`(law·admrul)나
  `조문.조`(ordin) 같은 구조화 배열이 있으면 그대로 쓰고, 없으면 텍스트를 모아
  정규식으로 파싱한다.
- **`admrul` 상세 응답의 본문이 "제N조" 형식이 아니라 Ⅰ./1./가. 같은 개요식
  번호로 된 경우, 조/항/호 구조로 나누지 않고 번호 없는 원문 텍스트 하나로
  돌려준다.** (실제 확인: 「식품등의 표시기준」의 53KB 본문 전체가 이 형식이다.)
  구조를 억지로 지어내는 것보다 있는 그대로 보여주는 편이 안전하다는 판단이다.
  `articles`/`export` 결과에서 조문번호가 `None`이면 이 경우다.
- **자치법규(`ordin`)는 항(①②③)까지는 복원하지만 호(1. 2. 3.)는 항 텍스트
  안에 그대로 남는다.** ordin 원문은 항 사이에 줄바꿈이 전혀 없이 붙어 오는데,
  원문자는 한국 법령에서 항의 시작에만 쓰여 안전하게 줄을 나눌 수 있지만,
  숫자+마침표(1., 2.)는 날짜("2017.3.6")·목록과 헷갈릴 위험이 있어 같은
  방식으로 나누지 않는다. 호 내용 자체는 온전히 들어있으니 필요하면 텍스트에서
  직접 확인할 것.
...
- **검색 결과는 기본 20건까지만 보여준다.** 전체 건수가 더 많으면
  `[N~M번째 표시, 전체 K건]` 안내와 함께 `--page`/`--display` 사용법을
  알려준다 — 조용히 잘리지 않는다.
- **법정 정의처럼 정확도가 중요한 텍스트는 파싱 결과를 원본과 대조 확인할 것.**

### 향후 확장 후보

- **위임법령 조회 API** — 법령/행정규칙/자치법규 간 인용·위임 관계를
  조항호목 단위로 제공하는 별도 API. 현재 스킬은 다루지 않는다.
- **지능형 법령검색 시스템(`target=aiSearch`) 검색 API** — 법령/행정규칙의
  조문과 별표를 한 번에 검색하는 별도 엔드포인트. 필드명(조문번호·
  조문가지번호·별표서식번호·별표서식가지번호 등)이 이 스킬이 이미 쓰는
  디코딩 방식과 동일해 연동 자체는 어렵지 않을 것으로 보이나, 현재는
  구현하지 않았다.
